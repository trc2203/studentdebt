...
salaries = linspace(20000,100000,1000);
repaidTotals = []
for salary = salaries % starting annual salary in £
    ...
    outerLoopInd=1;
    fprintf('For a starting salary of £%d growing at %d%% per year:\n',salary,payRise)
    ...
    while month<=duration
        ...
        if undBal>0
            ...
            if undBal<0
                undBal=0; % balance can't be negative, to account for slight overpayment on last instalment
            end
        else
            undRepay=0;
        end
